require('../dist/scripts/public.prerender.development');

const blockProps = {
  'product': {
    'id': 36109,
    'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
    'is_sale': false,
    'has_ordering_goods': true,
    'is_label_new': false,
    'is_run_out': false,
    'is_sold': false,
    'selling_by_weight': false,
    'weight_of_price': null,
    'index_image': {
      'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
      'width': 600,
      'height': 600
    },
    'second_image': {
      'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/70608/f1f2c7b9-a65e-4d90-984a-43eacfde3542.jpg',
      'width': 640,
      'height': 672
    },
    'public_url': 'http://saharok.stage.kiiiosk.ru/products/36109-koltso-na-vtoruyu-falangu-mmkxss-jewelry',
    'short_details': 'Серебро',
    'goods': [{
        'id': 32724,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS8zMjcyNA',
        'is_sale': false,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'id': 42608,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MjYwOA',
        'is_sale': false,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'id': 32725,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS8zMjcyNQ',
        'is_sale': false,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'id': 42609,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MjYwOQ',
        'is_sale': false,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'id': 43421,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MzQyMQ',
        'is_sale': false,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'id': 43422,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MzQyMg',
        'is_sale': false,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      }
    ]
  },
  'showCartButton': false,
  'showQuantity': false,
  'i18n': {
    'locale': 'ru',
    'translations': {
      'vendor': {
        'not_published': 'Магазин временно не работает',
        'made_in_kiiiosk': 'Сделано на «Киоске»',
        'tasty_product_button_text': 'Купить',
        'entities': {
          'product': '%{count} товар',
          'product_2': '%{count} товара',
          'product_5': '%{count} товаров'
        },
        'badges': {
          'new': 'Новинка',
          'sale_percent': 'SALE - %{percent}%',
          'not_available': 'Не продается',
          'sale': 'SALE',
          'sold': 'Продано'
        },
        'gallery': {
          'close': 'Закрыть',
          'next': 'Вперёд',
          'prev': 'Назад'
        },
        'search': {
          'results_title': 'Результаты поиска (%{count})',
          'nothing_found': 'К сожалению, ничего не найдено.'
        },
        'seconds_count': '%{count} секунду',
        'seconds_count_2': '%{count} секунды',
        'seconds_count_5': '%{count} секунд',
        'activemodel': {
          'errors': {
            'models': {
              'vendor_order_form': {
                'invalid_phone': 'Неверный формат телефона. Прим.: +7 913 123 32 10',
                'attributes': {
                  'address': {
                    'blank': 'не может быть пустым'
                  },
                  'email': {
                    'email': 'не является email адресом'
                  },
                  'city_title': {
                    'blank': 'не может быть пустым'
                  },
                  'phone': {
                    'blank': 'не может быть пустым'
                  },
                  'name': {
                    'blank': 'не может быть пустым'
                  },
                  'public_offer_accepted': {
                    'blank': 'Необходимо принять условия публичной оферты'
                  }
                }
              }
            }
          }
        },
        'errors': {
          'cart': {
            'empty': 'В корзине нет товаров'
          },
          'coupon': {
            'not_found': 'Несуществующий промокод %{code}',
            'expired': 'Промокод просрочен %{code}'
          },
          'order': {
            'invalid_form': 'Заказ еще не принят, исправьте ошибки в форме',
            'has_unorderable_goods': 'В заказе лежат недоступные товары',
            'no_items': 'В заказе нет товаров',
            'unavailable_currency': 'Невозможно сделать заказ в выбранной валюте'
          }
        },
        'placeholders': {
          'coupon': 'Промо-код (если есть)',
          'search': 'Поиск'
        },
        'alerts': {
          'confirm': 'Точно очистить?'
        },
        'flashes': {
          'good_added_to_basket': 'Украшение "%{title}" добавлено в корзину'
        },
        'mails': {
          'signature': 'С уважением, интернет-магазин "%{name}"'
        },
        'auto_menu_items': {
          'blog': 'Блог'
        },
        'top_banner': {
          'default_content': 'Мы открылись!'
        },
        'order': {
          'fields': {
            'phone': 'Телефон',
            'name': 'Полное имя',
            'email': 'Email',
            'city_title': 'Город доставки',
            'address': 'Адрес доставки',
            'comment': 'Оставьте свой телефон и имя, наш сотрудник свяжется с вами для уточнения информации о заказе',
            'coupon_code': 'Промокод'
          },
          'placeholders': {
            'city_title': 'Москва',
            'phone': 'Например, +7 913 123 32 10',
            'name': 'Имя',
            'address': 'Например, Академика Вавилова 12-10',
            'comment': 'О чем нам нужно знать',
            'email': 'Email'
          },
          'submit': 'Оформить заказ',
          'pickup_address': 'Адрес самовывоза:',
          'title': 'Заказ №%{number}',
          'payment_type': 'Способ оплаты: %{title}',
          'delivery_type': 'Способ доставки: %{title}',
          'free_delivery_text_html': 'Бесплатная доставка при заказе свыше <nobr>%{free_delivery_threshold}</nobr>',
          'checkout_free_delivery_text_html': 'Бесплатно при заказе свыше <nobr>%{free_delivery_threshold}</nobr>',
          'go_back': 'Вернуться назад',
          'next': 'Продолжить',
          'delivery_tracking_id': 'Трекинг-номер доставки: %{number}',
          'check_state': 'Проверить статус доставки',
          'public_offer_accepted_html': 'Согласен с условиями <u><a href="%{url}" target="_blank">публичной оферты</a></u>',
          'created': {
            'title': 'Спасибо за заказ',
            'desc_html': 'В ближайшее время менеджер свяжется с вами для подтверждения заказа. <br> Ваш телефон %{phone} <br> Ваш заказ %{link} на сумму %{price}\n'
          },
          'new': {
            'delivery_title': 'Выберите тип доставки.',
            'payment_title': 'Способы оплаты',
            'contacts_title': 'Введите данные',
            'sum': 'на сумму'
          },
          'redirect': 'В течение 5-и секунд вы будете переправлены на страницу оплаты.',
          'redirecting': 'Переправляю на сайт оплаты',
          'go_to_payment': 'Перейти к оплате',
          'continue_shopping': 'Продолжить покупки',
          'contents': 'Состав заказа:',
          'delivery_price': 'Стоимость доставки',
          'pay': 'Оплатить'
        },
        'blog': {
          'read_more': 'Читать далее..'
        },
        'notice': {
          'catalog_filter': {
            'selected_products': 'Выбрано вариантов',
            'show_products': 'Показать'
          }
        },
        'coupon': {
          'discount': 'Скидка - %{discount}',
          'invalid': 'Купон "%{value}" НЕ действующий',
          'checking': 'Проверяю купон..',
          'error': 'Ошибка при проверке купона. Попробуйте еще раз'
        },
        'cart': {
          'basket_button': 'Корзина',
          'amount': 'Количество',
          'weight': 'Вес',
          'overall': 'Итого:',
          'title': 'Корзина',
          'clear': 'Очистить корзину',
          'not_available': 'Больше не продается',
          'empty': 'Пока тут ничего нет'
        },
        'wishlist': {
          'add_item': 'Добавить в список желаний',
          'wishlist_button': 'Список желаний',
          'private_title': 'Ваш список желаний',
          'foreign_title': 'Чей-то список желаний',
          'empty': 'Пока тут ничего нет',
          'no_price': 'Не продается'
        },
        'packaging': {
          'add': 'Добавить',
          'add_gift_package': 'Добавить фактурную подарочную упаковку, перевязанную шелковой лентой',
          'no_package': 'Не добавлять'
        },
        'payment': {
          'w1': {
            'failure': {
              'title': 'Оплата не прошла',
              'desc': 'Оплата заказа не была произведена'
            },
            'success': {
              'title': 'Спасибо за заказ',
              'desc': 'Оплата заказа была произведена'
            }
          }
        },
        'category': {
          'continue_shopping': 'Продолжить покупки',
          'empty': 'Ой, а здесь ничего нет. Пока.'
        },
        'dictionary_entity': {
          'continue_shopping': 'Продолжить покупки',
          'empty': 'Ой, а здесь ничего нет. Пока.'
        },
        'similar_product': {
          'title': 'С этим украшением покупают также'
        },
        'product': {
          'title': 'Товар',
          'blank_price': 'Цена неизвестна',
          'run_out': 'Нет в наличии',
          'not_available': 'Не продается',
          'available': 'Можно купить',
          'sold_out_message_html': '&laquo;%{title}&raquo; сейчас не продается',
          'nothing_found_by_criteria': 'Сожалеем, ничего не найдено. Попробуйте изменить критерии поиска.',
          'category_title': 'Категория',
          'show_other_products': 'Посмотреть другие товары',
          'show_all': 'Показать все',
          'weight': 'Вес',
          'kg': 'кг.'
        },
        'properties': {
          'empty': 'Нет характеристик для вывода'
        },
        'property': {
          'unknown_type': 'Неизвестный тип характеристики',
          'defaults': {
            'size': 'Размер',
            'color': 'Цвет'
          }
        },
        'pages': {
          'titles': {
            'cart': 'Корзина',
            'order': 'Оформление заказа',
            'payment': 'Оплата',
            'payment_success': 'Успешная оплата',
            'payment_error': 'Ошибка оплаты'
          }
        },
        'filter': {
          'availability': 'Доступность',
          'price_range': 'Ценовой диапазон',
          'expand_button': {
            'expand': 'Развернуть список',
            'turn': 'Свернуть список'
          },
          'selected_options': {
            'availability': 'Доступность',
            'price_range': 'Цена от %{from} до %{to} %{currency}'
          },
          'selected_availability': {
            'all': 'Все',
            'available': 'В наличии',
            'run_out': 'Под заказ',
            'sale': 'Распродажа',
            'unknown': 'unknown'
          }
        },
        'button': {
          'to_cart': 'Добавить в корзину %{title}',
          'select_good': 'Выберите характеристику',
          'go_wishlist': 'Перейти в "Список желаний"',
          'to_wishlist': 'Добавить в "Список желаний"',
          'already': 'Уже в корзине',
          'disable_with': {
            'sending': 'Отправляем...',
            'saving': 'Сохраняем...',
            'waiting': 'Подождите...',
            'adding': 'Добавляем...'
          }
        },
        'products': {
          'others': 'Остальные',
          'nothing_found_by_criteria': 'По данным критериям ничего не найдено'
        },
        'client': {
          'auth': 'Личный кабинет',
          'no_account': 'Нет аккаунта?',
          'logged_in_with': 'Вы уже авторизованы под именем<br /> <b>%{name}</b>.',
          'continue_logged': 'Продолжить под этим именем..',
          'create_account': 'Создать',
          'logout': 'Выйти',
          'cabinet': {
            'title': 'Личный кабинет',
            'button': 'Кабинет',
            'create': 'Создаем личный кабинет',
            'send_pin_code': 'Выслать пин-код',
            'disable_with': 'Высылаем...'
          }
        },
        'money': {
          'unknown_iso_code': 'Неизвестный тип валюты %{isoCode}'
        }
      }
    }
  },
  'formAuthenticity': {
    'token': 'REFKvsEf/pWfNDoRM3LPVHNgTIY5d32YR4P/xACndXk=',
    'field': 'authenticity_token'
  }
};

const cardProps = {
  'product': {
    'id': 36109,
    'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0LzM2MTA5',
    'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
    'article': null,
    'has_ordering_goods': true,
    'is_sale': false,
    'is_label_new': false,
    'is_run_out': false,
    'is_sold': false,
    'selling_by_weight': false,
    'weight_of_price': null,
    'description': 'Украшения бренда MMKXSS jewelry - образец работы с минималистичными формами. Четкие линии, ничего лишнего. \n\nКольца на вторую фалангу могут быть как матовыми, так и глянцевыми, а значит получатся интересные сочетания и по форме, и по фактуре. \nУкрашения отлично подходят в качестве базовых, они легко сочетаются с другими вещами из серебра и и не только.',
    'sale_percent': null,
    'main_category': {
      'id': 1587,
      'name': 'Кольца',
      'is_root': false,
      'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1587-ukrasheniya-koltsa',
      'parent': {
        'id': 2926,
        'name': 'Украшения',
        'is_root': false,
        'public_url': 'http://saharok.stage.kiiiosk.ru/categories/2926-ukrasheniya',
        'parent': {
          'id': 1562,
          'name': 'Корневая категория',
          'is_root': true,
          'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1562-kornevaya-kategoriya',
          'parent': null
        }
      }
    },
    'categories': [{
        'id': 1730,
        'name': 'Главная',
        'is_root': false,
        'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1730-glavnaya',
        'parent': {
          'id': 1562,
          'name': 'Корневая категория',
          'is_root': true,
          'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1562-kornevaya-kategoriya',
          'parent': null
        }
      },
      {
        'id': 2904,
        'name': 'Хиты продаж',
        'is_root': false,
        'public_url': 'http://saharok.stage.kiiiosk.ru/categories/2904-hity-prodazh',
        'parent': {
          'id': 1562,
          'name': 'Корневая категория',
          'is_root': true,
          'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1562-kornevaya-kategoriya',
          'parent': null
        }
      },
      {
        'id': 2486,
        'name': 'Кольца на фалангу',
        'is_root': false,
        'public_url': 'http://saharok.stage.kiiiosk.ru/categories/2486-ukrasheniya-koltsa-na-falangu',
        'parent': {
          'id': 2926,
          'name': 'Украшения',
          'is_root': false,
          'public_url': 'http://saharok.stage.kiiiosk.ru/categories/2926-ukrasheniya',
          'parent': {
            'id': 1562,
            'name': 'Корневая категория',
            'is_root': true,
            'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1562-kornevaya-kategoriya',
            'parent': null
          }
        }
      },
      {
        'id': 1587,
        'name': 'Кольца',
        'is_root': false,
        'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1587-ukrasheniya-koltsa',
        'parent': {
          'id': 2926,
          'name': 'Украшения',
          'is_root': false,
          'public_url': 'http://saharok.stage.kiiiosk.ru/categories/2926-ukrasheniya',
          'parent': {
            'id': 1562,
            'name': 'Корневая категория',
            'is_root': true,
            'public_url': 'http://saharok.stage.kiiiosk.ru/categories/1562-kornevaya-kategoriya',
            'parent': null
          }
        }
      }
    ],
    'goods': [{
        'title': 'размер: 13',
        'article': null,
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS8zMjcyNA',
        'is_ordering': true,
        'is_run_out': false,
        'is_sale': false,
        'is_preorder': false,
        'image': {
          'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
          'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
          'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
        },
        'attributes': {
          '313': '13'
        },
        'quantity': 6,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'title': 'размер: 13,5',
        'article': null,
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MjYwOA',
        'is_ordering': true,
        'is_run_out': false,
        'is_sale': false,
        'is_preorder': false,
        'image': {
          'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
          'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
          'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
        },
        'attributes': {
          '313': '13,5'
        },
        'quantity': 20,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'title': 'размер: 14',
        'article': null,
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS8zMjcyNQ',
        'is_ordering': true,
        'is_run_out': false,
        'is_sale': false,
        'is_preorder': false,
        'image': {
          'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
          'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
          'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
        },
        'attributes': {
          '313': '14'
        },
        'quantity': 6,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'title': 'размер: 14,5',
        'article': null,
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MjYwOQ',
        'is_ordering': true,
        'is_run_out': false,
        'is_sale': false,
        'is_preorder': false,
        'image': {
          'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
          'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
          'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
        },
        'attributes': {
          '313': '14,5'
        },
        'quantity': 9,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'title': 'размер: 15',
        'article': null,
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MzQyMQ',
        'is_ordering': true,
        'is_run_out': false,
        'is_sale': false,
        'is_preorder': false,
        'image': {
          'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
          'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
          'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
        },
        'attributes': {
          '313': '15'
        },
        'quantity': 1,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      },
      {
        'title': 'размер: 15,5',
        'article': null,
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS80MzQyMg',
        'is_ordering': false,
        'is_run_out': true,
        'is_sale': false,
        'is_preorder': false,
        'image': {
          'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
          'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
          'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
        },
        'attributes': {
          '313': '15,5'
        },
        'quantity': 0,
        'price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 140000,
          'currency_iso_code': 'RUB'
        }
      }
    ],
    'images': [{
        'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/53578/a6e3499c-9252-46e0-8c59-b1615d1e9315.jpg',
        'uid': 'b2a73840da4e4eb4ae6848993895146700854a1c2a41780237b870fab719745c'
      },
      {
        'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/70608/f1f2c7b9-a65e-4d90-984a-43eacfde3542.jpg',
        'uid': '18858a11c3c358d1896fce69943b18c444f1a6c1f82ad8285f2efb4dd04c4024'
      },
      {
        'title': 'Кольцо на вторую фалангу MMKXSS JEWELRY',
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/59873/36aab696-fca6-44a4-9400-43b08f0e0733.jpg',
        'uid': 'ea3bf237b2bfcf70091557a5702ed4c82283d0de4293108b32bda37a32d68c92'
      }
    ],
    'properties': [],
    'attributes': [{
        'property_id': 371,
        'type': 'AttributeDictionary',
        'title': 'Бренд',
        'value': 'MMKXSS JEWELRY',
        'products_url': 'http://saharok.stage.kiiiosk.ru/entities/10032-mmkxss-jewelry'
      },
      {
        'property_id': 373,
        'type': 'AttributeDictionary',
        'title': 'Материал',
        'value': 'Серебро',
        'products_url': 'http://saharok.stage.kiiiosk.ru/entities/468-serebro'
      }
    ],
    'custom_product_html': null,
    'text_blocks': [],
    'video_embed_html': null
  },
  'similarProducts': [{
      'id': 20811,
      'title': 'Браслет HIMÈRE',
      'is_sale': false,
      'has_ordering_goods': true,
      'is_label_new': false,
      'is_run_out': false,
      'is_sold': false,
      'selling_by_weight': false,
      'weight_of_price': null,
      'index_image': {
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/61004/52cc2d94-12d6-4b29-bd26-d6b2bd1422aa.jpg',
        'width': 1500,
        'height': 1500
      },
      'second_image': {
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/33062/d1dff69c-511a-4865-b487-5df7a99f96cf.jpg',
        'width': 640,
        'height': 640
      },
      'public_url': 'http://saharok.stage.kiiiosk.ru/products/20811-braslet-him-re',
      'short_details': 'Шелковая нить, Серебро, Серебро',
      'goods': [{
        'id': 26396,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS8yNjM5Ng',
        'is_sale': false,
        'price': {
          'cents': 200000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 200000,
          'currency_iso_code': 'RUB'
        }
      }]
    },
    {
      'id': 24064,
      'title': 'Кафф ANNA.M.OBJECTS',
      'is_sale': false,
      'has_ordering_goods': true,
      'is_label_new': false,
      'is_run_out': false,
      'is_sold': false,
      'selling_by_weight': false,
      'weight_of_price': null,
      'index_image': {
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/9474/c78b5c81-c905-424b-a2cd-579ec676efc8.jpg',
        'width': 1500,
        'height': 1500
      },
      'second_image': {
        'url': 'http://assets.stage.kiiiosk.ru/uploads/shop/68/uploads/product_image/image/71336/875f3d6a-a775-4172-a064-9f092a0bfd02.jpg',
        'width': 750,
        'height': 875
      },
      'public_url': 'http://saharok.stage.kiiiosk.ru/products/24064-kaff-anna-m-objects',
      'short_details': 'Серебро',
      'goods': [{
        'id': 28042,
        'type': 'ProductItem',
        'global_id': 'Z2lkOi8vbWVyY2hhbnRseS9Qcm9kdWN0SXRlbS8yODA0Mg',
        'is_sale': false,
        'price': {
          'cents': 120000,
          'currency_iso_code': 'RUB'
        },
        'actual_price': {
          'cents': 120000,
          'currency_iso_code': 'RUB'
        }
      }]
    }
  ],
  'i18n': {
    'locale': 'ru',
    'translations': {
      'vendor': {
        'not_published': 'Магазин временно не работает',
        'made_in_kiiiosk': 'Сделано на «Киоске»',
        'tasty_product_button_text': 'Купить',
        'entities': {
          'product': '%{count} товар',
          'product_2': '%{count} товара',
          'product_5': '%{count} товаров'
        },
        'badges': {
          'new': 'Новинка',
          'sale_percent': 'SALE - %{percent}%',
          'not_available': 'Не продается',
          'sale': 'SALE',
          'sold': 'Продано'
        },
        'gallery': {
          'close': 'Закрыть',
          'next': 'Вперёд',
          'prev': 'Назад'
        },
        'search': {
          'results_title': 'Результаты поиска (%{count})',
          'nothing_found': 'К сожалению, ничего не найдено.'
        },
        'seconds_count': '%{count} секунду',
        'seconds_count_2': '%{count} секунды',
        'seconds_count_5': '%{count} секунд',
        'activemodel': {
          'errors': {
            'models': {
              'vendor_order_form': {
                'invalid_phone': 'Неверный формат телефона. Прим.: +7 913 123 32 10',
                'attributes': {
                  'address': {
                    'blank': 'не может быть пустым'
                  },
                  'email': {
                    'email': 'не является email адресом'
                  },
                  'city_title': {
                    'blank': 'не может быть пустым'
                  },
                  'phone': {
                    'blank': 'не может быть пустым'
                  },
                  'name': {
                    'blank': 'не может быть пустым'
                  },
                  'public_offer_accepted': {
                    'blank': 'Необходимо принять условия публичной оферты'
                  }
                }
              }
            }
          }
        },
        'errors': {
          'cart': {
            'empty': 'В корзине нет товаров'
          },
          'coupon': {
            'not_found': 'Несуществующий промокод %{code}',
            'expired': 'Промокод просрочен %{code}'
          },
          'order': {
            'invalid_form': 'Заказ еще не принят, исправьте ошибки в форме',
            'has_unorderable_goods': 'В заказе лежат недоступные товары',
            'no_items': 'В заказе нет товаров',
            'unavailable_currency': 'Невозможно сделать заказ в выбранной валюте'
          }
        },
        'placeholders': {
          'coupon': 'Промо-код (если есть)',
          'search': 'Поиск'
        },
        'alerts': {
          'confirm': 'Точно очистить?'
        },
        'flashes': {
          'good_added_to_basket': 'Украшение "%{title}" добавлено в корзину'
        },
        'mails': {
          'signature': 'С уважением, интернет-магазин "%{name}"'
        },
        'auto_menu_items': {
          'blog': 'Блог'
        },
        'top_banner': {
          'default_content': 'Мы открылись!'
        },
        'order': {
          'fields': {
            'phone': 'Телефон',
            'name': 'Полное имя',
            'email': 'Email',
            'city_title': 'Город доставки',
            'address': 'Адрес доставки',
            'comment': 'Оставьте свой телефон и имя, наш сотрудник свяжется с вами для уточнения информации о заказе',
            'coupon_code': 'Промокод'
          },
          'placeholders': {
            'city_title': 'Москва',
            'phone': 'Например, +7 913 123 32 10',
            'name': 'Имя',
            'address': 'Например, Академика Вавилова 12-10',
            'comment': 'О чем нам нужно знать',
            'email': 'Email'
          },
          'submit': 'Оформить заказ',
          'pickup_address': 'Адрес самовывоза:',
          'title': 'Заказ №%{number}',
          'payment_type': 'Способ оплаты: %{title}',
          'delivery_type': 'Способ доставки: %{title}',
          'free_delivery_text_html': 'Бесплатная доставка при заказе свыше <nobr>%{free_delivery_threshold}</nobr>',
          'checkout_free_delivery_text_html': 'Бесплатно при заказе свыше <nobr>%{free_delivery_threshold}</nobr>',
          'go_back': 'Вернуться назад',
          'next': 'Продолжить',
          'delivery_tracking_id': 'Трекинг-номер доставки: %{number}',
          'check_state': 'Проверить статус доставки',
          'public_offer_accepted_html': 'Согласен с условиями <u><a href="%{url}" target="_blank">публичной оферты</a></u>',
          'created': {
            'title': 'Спасибо за заказ',
            'desc_html': 'В ближайшее время менеджер свяжется с вами для подтверждения заказа. <br> Ваш телефон %{phone} <br> Ваш заказ %{link} на сумму %{price}\n'
          },
          'new': {
            'delivery_title': 'Выберите тип доставки.',
            'payment_title': 'Способы оплаты',
            'contacts_title': 'Введите данные',
            'sum': 'на сумму'
          },
          'redirect': 'В течение 5-и секунд вы будете переправлены на страницу оплаты.',
          'redirecting': 'Переправляю на сайт оплаты',
          'go_to_payment': 'Перейти к оплате',
          'continue_shopping': 'Продолжить покупки',
          'contents': 'Состав заказа:',
          'delivery_price': 'Стоимость доставки',
          'pay': 'Оплатить'
        },
        'blog': {
          'read_more': 'Читать далее..'
        },
        'notice': {
          'catalog_filter': {
            'selected_products': 'Выбрано вариантов',
            'show_products': 'Показать'
          }
        },
        'coupon': {
          'discount': 'Скидка - %{discount}',
          'invalid': 'Купон "%{value}" НЕ действующий',
          'checking': 'Проверяю купон..',
          'error': 'Ошибка при проверке купона. Попробуйте еще раз'
        },
        'cart': {
          'basket_button': 'Корзина',
          'amount': 'Количество',
          'weight': 'Вес',
          'overall': 'Итого:',
          'title': 'Корзина',
          'clear': 'Очистить корзину',
          'not_available': 'Больше не продается',
          'empty': 'Пока тут ничего нет'
        },
        'wishlist': {
          'add_item': 'Добавить в список желаний',
          'wishlist_button': 'Список желаний',
          'private_title': 'Ваш список желаний',
          'foreign_title': 'Чей-то список желаний',
          'empty': 'Пока тут ничего нет',
          'no_price': 'Не продается'
        },
        'packaging': {
          'add': 'Добавить',
          'add_gift_package': 'Добавить фактурную подарочную упаковку, перевязанную шелковой лентой',
          'no_package': 'Не добавлять'
        },
        'payment': {
          'w1': {
            'failure': {
              'title': 'Оплата не прошла',
              'desc': 'Оплата заказа не была произведена'
            },
            'success': {
              'title': 'Спасибо за заказ',
              'desc': 'Оплата заказа была произведена'
            }
          }
        },
        'category': {
          'continue_shopping': 'Продолжить покупки',
          'empty': 'Ой, а здесь ничего нет. Пока.'
        },
        'dictionary_entity': {
          'continue_shopping': 'Продолжить покупки',
          'empty': 'Ой, а здесь ничего нет. Пока.'
        },
        'similar_product': {
          'title': 'С этим украшением покупают также'
        },
        'product': {
          'title': 'Товар',
          'blank_price': 'Цена неизвестна',
          'run_out': 'Нет в наличии',
          'not_available': 'Не продается',
          'available': 'Можно купить',
          'sold_out_message_html': '&laquo;%{title}&raquo; сейчас не продается',
          'nothing_found_by_criteria': 'Сожалеем, ничего не найдено. Попробуйте изменить критерии поиска.',
          'category_title': 'Категория',
          'show_other_products': 'Посмотреть другие товары',
          'show_all': 'Показать все',
          'weight': 'Вес',
          'kg': 'кг.'
        },
        'properties': {
          'empty': 'Нет характеристик для вывода'
        },
        'property': {
          'unknown_type': 'Неизвестный тип характеристики',
          'defaults': {
            'size': 'Размер',
            'color': 'Цвет'
          }
        },
        'pages': {
          'titles': {
            'cart': 'Корзина',
            'order': 'Оформление заказа',
            'payment': 'Оплата',
            'payment_success': 'Успешная оплата',
            'payment_error': 'Ошибка оплаты'
          }
        },
        'filter': {
          'availability': 'Доступность',
          'price_range': 'Ценовой диапазон',
          'expand_button': {
            'expand': 'Развернуть список',
            'turn': 'Свернуть список'
          },
          'selected_options': {
            'availability': 'Доступность',
            'price_range': 'Цена от %{from} до %{to} %{currency}'
          },
          'selected_availability': {
            'all': 'Все',
            'available': 'В наличии',
            'run_out': 'Под заказ',
            'sale': 'Распродажа',
            'unknown': 'unknown'
          }
        },
        'button': {
          'to_cart': 'Добавить в корзину %{title}',
          'select_good': 'Выберите характеристику',
          'go_wishlist': 'Перейти в "Список желаний"',
          'to_wishlist': 'Добавить в "Список желаний"',
          'already': 'Уже в корзине',
          'disable_with': {
            'sending': 'Отправляем...',
            'saving': 'Сохраняем...',
            'waiting': 'Подождите...',
            'adding': 'Добавляем...'
          }
        },
        'products': {
          'others': 'Остальные',
          'nothing_found_by_criteria': 'По данным критериям ничего не найдено'
        },
        'client': {
          'auth': 'Личный кабинет',
          'no_account': 'Нет аккаунта?',
          'logged_in_with': 'Вы уже авторизованы под именем<br /> <b>%{name}</b>.',
          'continue_logged': 'Продолжить под этим именем..',
          'create_account': 'Создать',
          'logout': 'Выйти',
          'cabinet': {
            'title': 'Личный кабинет',
            'button': 'Кабинет',
            'create': 'Создаем личный кабинет',
            'send_pin_code': 'Выслать пин-код',
            'disable_with': 'Высылаем...'
          }
        },
        'money': {
          'unknown_iso_code': 'Неизвестный тип валюты %{isoCode}'
        }
      }
    }
  },
  'formAuthenticity': {
    'token': 'REFKvsEf/pWfNDoRM3LPVHNgTIY5d32YR4P/xACndXk=',
    'field': 'authenticity_token'
  },
  'isWishlisted': false,
  'hasWishlist': true,
  'hasComments': false,
  'disqusUrl': null,
  'wishlistUrl': 'http://saharok.stage.kiiiosk.ru/wishlist',
  'addWishlistUrl': 'http://saharok.stage.kiiiosk.ru/wishlist/items'
};

const logoProps = {
  'linkUrl': 'http://wannabe.vagrant.dev:3000',
  'logoText': 'Wannabe Jewelry Moscow',
  'imageAlt': 'Wannabe Jewelry Moscow',
  'logoUrl': null,
};

global.ReactDOMServer.renderToString(global.React.createElement(global.Logo, logoProps));
global.ReactDOMServer.renderToString(global.React.createElement(global.ProductBlock, blockProps));
global.ReactDOMServer.renderToString(global.React.createElement(global.ProductCard, cardProps));
global.ReactDOMServer.renderToString(global.React.createElement(global.CartContainer, require('../test/fixtures/cart/with-package')));
global.ReactDOMServer.renderToString(global.React.createElement(global.CartContainer, require('../test/fixtures/cart/select-package')));
global.ReactDOMServer.renderToString(global.React.createElement(global.OrderContainer, require('../test/fixtures/order/with-coupon')));
global.ReactDOMServer.renderToString(global.React.createElement(global.WishlistContainer, require('../test/fixtures/wishlist/synteticData')));
global.ReactDOMServer.renderToString(global.React.createElement(global.MenuTop, require('test/text/fixtures/menuTop/sample1')));

console.log('Prerendering public components test finished successfully');
